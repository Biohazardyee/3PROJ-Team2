...
export default function CommentsScreen() {
    ...
    useEffect((): () => void => {
        const showSub: EmitterSubscription = Keyboard.addListener("keyboardWillShow", (e): void => {
            setKeyboardHeight(e.endCoordinates.height);
        });

        const hideSub: EmitterSubscription = Keyboard.addListener("keyboardWillHide", (): void => {
            setKeyboardHeight(0);
        });
    ...
    const orderedComments: Comment[] = useMemo((): Comment[] => {
        const parents: Comment[] = comments.filter((c: Comment): boolean => !c.parent_id);
        const result: Comment[] = [];
        parents.forEach((parent: Comment): void => {
            result.push(parent);
            const replies: Comment[] = comments.filter((c: Comment): boolean => c.parent_id === parent.id);
            result.push(...replies);
        });
        return result;
    }, [comments]);
    ...
    const handleToggleLikeReview: () => Promise<void> = async (): Promise<void> => {
        if (!review) return;
        ...
        try {
            ...
            setReview((prev: Review | null) => (prev ? {...prev, isLiked, likes_count} : null));
        } catch (error) {
    ...
    const handleToggleLike: (commentId: string) => Promise<void> = async (commentId: string): Promise<void> => {
        try {
    ...
    const handleDeleteComment: (commentId: string) => void = (commentId: string): void => {
            ...
            {text: "Annuler", style: "cancel"},
            {
                ...
                onPress: async (): Promise<void> => {
                    try {
                        ...
                        setComments((prev: Comment[]): Comment[] => prev.filter((c: Comment): boolean => c.id !== commentId));
                    } catch (err) {
                        console.error(err);
                    }
                },
