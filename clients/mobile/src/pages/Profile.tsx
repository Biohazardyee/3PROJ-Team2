...
const ProfileScreen = () => {
    ...
    useEffect(() => {
        if (activeTab === "Recent Activity" && recentActivity.length === 0 && userProfil?.id) {
    ...
    const checkFollowStatus = async (targetUserId: string, currentUserId: string) => {
        try {
            const response = await apiClient.get(`/follows/following/${currentUserId}`);
            const followingList = response.data.data || [];

            const alreadyFollowing = followingList.some((item: any) =>
    ...
    /**
     * INTERROGE LA DB POUR LES COMPTEURS
     * Cette fonction garantit que les chiffres sont persistants et réels
     */
    const fetchFollowCounts = async (userId: string) => {
        try {
            ...
            // Selon ton API, soit on lit .count, soit on compte la longueur du tableau dans .data
            const followersCount = resFollowers.data.count ?? (resFollowers.data.data?.length || 0);
            const followingCount = resFollowing.data.count ?? (resFollowing.data.data?.length || 0);
    ...
    const handleFollowToggle = async () => {
        ...
        // UI Optimiste (on change direct pour la fluidité)
        setIsFollowing(!previousStatus);
        setFollowCounts(prev => ({
            ...prev,
            followers: previousStatus ? Math.max(0, prev.followers - 1) : prev.followers + 1
        }));
        ...
        try {
            ...
            // Une fois l'action réussie, on refresh les vrais comptes pour être synchro
    ...
    // ... (Reste des fonctions fetchPlaylists, fetchFavoriteAlbums, fetchRecentActivity identiques)
    const fetchPlaylists = async (userId: string) => {
        try {
            const response = await apiClient.get(`/playlists/user/${userId}`);
            setPlaylists(response.data.playlists || response.data.data || response.data || []);
        } catch (error: any) { console.error("❌ Erreur Playlists :", error.response?.status); }
    };

    const fetchFavoriteAlbums = async (userId: string) => {
        try {
            const response = await apiClient.get(`/reviews/user/${userId}/top`);
            setFavoriteReviews(response.data.data || []);
        } catch (error: any) { console.error("❌ Erreur Favorite Albums :", error.response?.status); }
    };
    ...
    const fetchRecentActivity = async (offset: number, userId: string) => {
        ...
        try {
            ...
            const newItems = (response.data.data || []).map((review: any) => ({
                ...
                album: review.media?.content?.name,
                artist: review.media?.content?.artist,
                cover: review.media?.content?.cover,
        ...
        } catch (error: any) { console.error("❌ Erreur Activité :", error.response?.status);
    ...
    const handleLike = async (id: string) => {
        ...
        if (itemIndex === -1) { isInteracting.current = false; return; }
        ...
        } catch (error) { setRecentActivity(recentActivity);
        } finally { isInteracting.current = false; }
    };

    const handleScroll = (event: any) => {
        const { layoutMeasurement, contentOffset, contentSize } = event.nativeEvent;
        if (layoutMeasurement.height + contentOffset.y >= contentSize.height - 100 &&
            activeTab === "Recent Activity" && hasMoreActivity && !loadingMore && userProfil?.id) {
    ...
    return (
        <AuthGuardWrapper>
            <View style={styles.container}>
                <Header />
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContainer} onScroll={handleScroll} scrollEventThrottle={16}>
                    <View>
                        <Image source={{ uri: "https://img.freepik.com/free-vector/gradient-music-notes-background_23-2151320190.jpg?w=740" }} style={styles.banner} />
                        <View style={styles.profilePicOuter}>
                            <View style={styles.profilePicInner}>
                                {userProfil?.profile_picture ? (
                                    <Image source={{ uri: userProfil.profile_picture }} style={styles.fullImage} />
                                ) : (
                                    <Text style={styles.profileLetter}>{userProfil?.username?.substring(0, 2).toUpperCase()}</Text>
                                )}
                            </View>
                        </View>
                    </View>
                    ...
                    <View style={styles.contentPadding}>
                        ...
                        <Text style={styles.handle}>@{userProfil?.username?.toLowerCase()}</Text>
                        ...
                        {isOwnProfile ? (
                            ...
                            <TouchableOpacity style={[styles.editButton, isFollowing ? styles.followingButton : styles.followButton]} onPress={handleFollowToggle}>
                                <Ionicons name={isFollowing ? "checkmark-circle-outline" : "person-add-outline"} size={18} color="#fff" />
                                <Text style={styles.editButtonText}>{isFollowing ? "Following" : "Follow"}</Text>
                            </TouchableOpacity>
                        )}
                        <Text style={styles.bio}>{userProfil?.biography || "No biography yet."}</Text>
                    </View>
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                            {tabs.map((tab) => (
                                <TouchableOpacity key={tab} onPress={() => setActiveTab(tab)} style={[styles.tabItem, activeTab === tab && styles.tabItemActive]}>
                                    <Text style={[styles.tabText, activeTab === tab && styles.tabTextActive]}>{tab === "Playlists" ? `Playlists (${playlists.length})` : tab}</Text>
                                </TouchableOpacity>
                            ))}
                        </ScrollView>
                    </View>
                            ...
                            <View style={styles.albumGrid}>
                                {favoriteReviews.map((item) => (
                                    <View key={item.id} style={styles.cardWrapper}>
                                        <AlbumCard id={item.media_id} title={item.media?.content?.name} artist={item.media?.content?.artist} rating={item.rating.toString()} cover={item.media?.content?.cover} />
                                    </View>
                                ))}
                            </View>
                        )}
                        ...
                        {activeTab === "Playlists" && (
                            <View style={styles.playlistList}>
                                {playlists.map((playlist) => (
                                    <TouchableOpacity key={playlist.id} style={styles.playlistItem} onPress={() => router.push({ pathname: "/playlistdetails", params: { id: playlist.id, title: playlist.name } })}>
                                        <View style={styles.playlistIconBox}>
                                            {playlist.image_url ? <Image source={{ uri: playlist.image_url.startsWith('data') ? playlist.image_url : `data:image/jpeg;base64,${playlist.image_url}` }} style={styles.playlistImage} /> : <Ionicons name="musical-notes-outline" size={24} color="#4A90E2" />}
                                        </View>
                            ...
                            <View style={styles.postsList}>
                                {recentActivity.map((item) => (
                                    <View key={item.id} style={styles.card}>
                                        <Text style={styles.cardUserName}>{item.user_name} <Text style={styles.actionText}>rated an album</Text></Text>
                                        <Text style={styles.postContent}>{item.content}</Text>
                                        <TouchableOpacity onPress={() => handleLike(item.id)}>
                                            <Ionicons name={item.isLiked ? "heart" : "heart-outline"} size={20} color={item.isLiked ? "#ec4899" : "#9ca3af"} />
                                        </TouchableOpacity>
                                    </View>
                                ))}
                            </View>
...
const styles = StyleSheet.create({
    ...
    loaderContainer: { flex: 1, backgroundColor: "#0f0f1e", justifyContent: "center", alignItems: "center" },
    scrollContainer: { paddingBottom: 100 },
    banner: { width: "100%", height: 160 },
    profilePicOuter: { marginTop: -55, marginLeft: 20, borderWidth: 4, borderColor: "#0f0f1e", borderRadius: 60, width: 110, height: 110, overflow: "hidden", backgroundColor: "#0f0f1e" },
    profilePicInner: { flex: 1, backgroundColor: "#4A90E2", justifyContent: "center", alignItems: "center" },
    ...
    editButton: { flexDirection: "row", paddingVertical: 12, borderRadius: 10, justifyContent: "center", alignItems: "center", borderWidth: 1, marginBottom: 15 },
    ...
    tabContainer: { marginTop: 15, borderBottomWidth: 1, borderBottomColor: "#1c1c2e" },
    ...
    playlistList: { gap: 12 },
    playlistItem: { flexDirection: "row", alignItems: "center", backgroundColor: "#1c1c2e", padding: 12, borderRadius: 12, gap: 15 },
    playlistIconBox: { width: 55, height: 55, backgroundColor: "#2a2a40", borderRadius: 8, justifyContent: "center", alignItems: "center", overflow: "hidden" },
