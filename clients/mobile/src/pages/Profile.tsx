...
import {useRouter, useLocalSearchParams, Router, UnknownOutputParams} from "expo-router";
...
const ProfileScreen = () => {
    ...
    const fetchProfile: (userId: string) => Promise<void> = async (userId: string): Promise<void> => {
        try {
    ...
    const fetchFollowCounts: (userId: string) => Promise<void> = async (userId: string): Promise<void> => {
        try {
    ...
    const handleFollowToggle: () => Promise<void> = async (): Promise<void> => {
        ...
        setFollowCounts((prev: { followers: number, following: number }): { followers: number, following: number } => ({
            ...prev,
    ...
    const fetchPlaylists: (userId: string) => Promise<void> = async (userId: string): Promise<void> => {
        try {
    ...
    const fetchFavoriteAlbums: (userId: string) => Promise<void> = async (userId: string): Promise<void> => {
        try {
    ...
    const handleLike: (id: string) => Promise<void> = async (id: string): Promise<void> => {
        ...
        isInteracting.current = true;
        const itemIndex: number = recentActivity.findIndex((f): boolean => f.id === id);
        if (itemIndex === -1) {
            isInteracting.current = false;
            return;
        }
