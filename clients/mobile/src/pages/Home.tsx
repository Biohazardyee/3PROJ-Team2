...
const Home: React.FC = () => {
  ...
  const log = (message: string) => {
    console.log(message);
    setDebugLogs((prev) => [...prev.slice(-20), message]); // max 20 logs
  };

  const handleSearch: () => Promise<void> = async (): Promise<void> => {
    const query: string = searchQuery.trim();

    log("🔍 handleSearch triggered");
    log("📦 query = " + query);
    ...
    try {
      ...
      log("📊 rawData length = " + rawData.length);

      if (!Array.isArray(rawData)) {
        log("⚠️ rawData is not array");
        return;
      }

      const fetchedAlbums = rawData.map((a: any) => {
        const artistName =
          typeof a.artist === "string" ? a.artist : a.artist?.name || "Inconnu";
      ...
      log("🎯 fetchedAlbums = " + fetchedAlbums.length);

      try {
        log("🔄 syncing to backend...");
        ...
        log("📦 syncedAlbums = " + syncedAlbums.length);

        const finalAlbums = syncedAlbums.length
          ? syncedAlbums.map((a: any) => ({
              id: a.id,
              apiId: a.api_id,
              album: a.name,
              artist: a.artist,
