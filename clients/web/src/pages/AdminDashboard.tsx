      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <header className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 dark:text-gray-600 text-lg mt-1 font-medium">Gérer les utilisateurs et le contenu</p>
          </div>
        </header>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <button className="flex items-center justify-center gap-2 bg-[#1a1d26] dark:bg-white border border-slate-800 dark:border-gray-200 hover:border-slate-600 dark:hover:border-gray-400 px-6 py-3.5 rounded-xl text-sm font-bold text-white dark:text-gray-700 transition-all">
                   <Ban size={18} className="text-slate-400 dark:text-gray-400" /> Utilisateurs Bannis
                </button>
              </div>
              <UserModerationCard />
            </div>
              ...
              {REPORTS.map((report) => (
                <div key={report.id} className="bg-[#1a1d26] dark:bg-white border border-slate-800 dark:border-gray-200 rounded-2xl p-6 shadow-lg border-l-4 border-l-rose-500">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className="text-lg font-bold text-white dark:text-gray-900">Signalement #{report.id}</h3>
                          <span className="bg-rose-500 text-white text-[10px] font-bold px-2 py-0.5 rounded uppercase">{report.status}</span>
                          <span className="bg-slate-800 dark:bg-gray-100 text-slate-300 dark:text-gray-600 text-[10px] font-bold px-2 py-0.5 rounded uppercase">{report.type}</span>
                        </div>
                        <p className="text-slate-500 dark:text-gray-500 text-xs">Signalé par <span className="text-slate-300 dark:text-gray-800 font-medium">{report.reportedBy}</span> • {report.timeAgo}</p>
                      </div>
                    </div>
                  </div>

                  <div className="bg-[#0f1117] dark:bg-gray-50 border border-slate-800 dark:border-gray-200 rounded-xl p-4 mb-6">
                    <p className="text-sm text-slate-300 dark:text-gray-700 mb-2 font-medium"><strong className="text-white dark:text-gray-900">Utilisateur:</strong> {report.target}</p>
                    <p className="text-sm text-slate-300 dark:text-gray-700 font-medium"><strong className="text-white dark:text-gray-900">Raison:</strong> {report.reason}</p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    <button className="bg-slate-800 dark:bg-gray-200 dark:text-gray-700 hover:bg-slate-700 dark:hover:bg-gray-300 text-white px-4 py-2 rounded-lg text-xs font-bold transition-all">Contacter l'utilisateur</button>
                    ...
                    <button className="text-slate-500 dark:text-gray-400 hover:text-white dark:hover:text-gray-900 px-4 py-2 rounded-lg text-xs font-bold transition-all ml-auto">Rejeter</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {activeTab === 'Analyses' && (
            <div className="bg-[#1a1d26] dark:bg-white border border-slate-800 dark:border-gray-200 rounded-2xl p-20 text-center animate-in fade-in duration-300">
              ...
              <h2 className="text-xl font-bold text-slate-500 dark:text-gray-400">Analyses sera bientôt disponible...</h2>
            </div>
          )}
        </div>
