...
const Feed: React.FC = () => {
  ...
  const displayedFeed = feedItems
    .filter(item => {
      // Affiche les reviews des utilisateurs suivis dans l'onglet suivis
      if (activeTab === 'following' && !item.isFollowing) {
        return false;
      }
      ...
      if (searchQuery.trim()) {
        ...
        // Si aucun des champs ne correspond à la recherche, on masque l'item
    ...
    .sort((a, b) => {
      // Filtrage pour l'onglet découverte (les plus likés d'abord)
      if (activeTab === 'trending') {
        return b.likes - a.likes;
      }
      // Sinon, on garde l'ordre classique (tri par ID)
      return a.id - b.id;
    });
