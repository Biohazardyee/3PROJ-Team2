...
import { isValidFloatRating, updateMediaAverageRating } from "./review.helper.js"
import {
    ReviewAddDto,
...
export class ReviewService {
    ...
    async create(data: ReviewAddDto): Promise<ReviewResponseAddDto> {
        ...
        const review = await PrismaDb.$transaction(async (tx) => {
            ...
            // MAJ de la moyenne
            await updateMediaAverageRating(tx, data.media_id);
    ...
    async getAll(userId?: string): Promise<ReviewWithMediaDto[]> {
        ...
        return reviews.map(review => {
            ...
            return {
                ...dto,
                isLiked: userId ? review.likes.some(l => l.user_id === userId) : false,
    ...
    async update(id: string, data: ReviewUpdateDto): Promise<ReviewResponseDto> {
        ...
        if (data.title !== undefined) {
            if (isEmptyString(data.title)) throw new BadRequest('Title cannot be empty');
