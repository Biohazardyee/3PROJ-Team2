...
export class ReviewService {
    async create(data: ReviewAddDto): Promise<ReviewResponseAddDto> {
        ...
        const realMediaId: string = media.id;
        ...
        if (isAllowed && tokens.length > 0) {
            ...
            ).catch((err): void => console.error("Broadcast failed:", err));
        }
    ...
    async getAll(userId?: string): Promise<ReviewWithMediaDto[]> {
        ...
        return reviews.map((review) => {
            const dto: ReviewWithMediaDto = reviewMapper.toReviewWithMediaDto(review);
            return {
                ...dto,
                isLiked: userId
                    ? review.likes.some((l): boolean => l.user_id === userId)
                    : false,
    ...
    async getTopAlbumsByUser(userId: string): Promise<ReviewWithMediaDto[]> {
        ...
        return topReviews.map((review) => {
            let mediaContent: JsonValue = review.media?.content;
    ...
    async getByMediaId(mediaId: string, userId?: string): Promise<ReviewWithMediaDto[]> {
